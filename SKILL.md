...
The index lives in `.folio/` under that root. **Add `.folio/` to the corpus's
ignore file before you index a repository** — folio will not do it for you, and
an index is derived from the corpus and belongs to whoever built it.

`--root` is repeatable and several indexes are ranked as one answer, each
reference naming the root it came from. A section two of them hold is returned
once. folio refuses by name any root it cannot show belongs with the others, so
a query that answers has answered from all of them.

Run `folio doctor` when results look wrong rather than absent. It exits 1 when
the endpoint fails one of the three things ranking depends on.
...
Five rows by default, `-l N` for more: a score, a `path:start-end`, and the
heading trail beneath it.
...
A row marked `(N sections)` covers several sections that touch and rank alike,
because that is the one read they describe.

A row marked `(stale)` sits in a file that moved after it was indexed, so treat
its range as approximate and locate its heading yourself before reading.

`--paths-only` prints one `path:start-end` per line and nothing else. Use it
when you are feeding ranges to a file reader rather than judging them; a stale
row is reported on stderr in that mode, so read stderr too.
...
Run `folio config` to see which of the flag, the environment, the corpus's
`folio.yaml` and the user's file answered. A corpus needing its own model — one
in a language the default was not trained for — carries `folio.yaml` at its root,
committed with it, so everyone indexes it the same way.
...
presence, `!key` tests absence, and `key!=value` also passes where the key is
absent — so a filter never silently drops what nobody annotated, and `!key` is
how you ask for those on purpose. `|` joins alternatives inside one argument and
separate `--where` flags are ANDed.

folio refuses a filter it cannot express instead of guessing, and names the
