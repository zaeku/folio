...
seconds of embedding at the 0.53 s per 32 inputs measured below.
...
6.3e-4 against the reference and Q4_k_m costs 0.024 (entry below) — 100× and
...
**This corrects the backend figure in the entry below.** That entry measured
...
fingerprint entry below. It is the most sensitive of the four to anything that
...
**The backend row is quantization-dependent**, which the entry above found after
this one was written. On F16 the same Metal-against-CPU comparison is 1.5e-6
...
below, which measured the same corpora while a long section was cut and results
...
873,797 characters that the entry below found in no vector are in vectors now,
...
same thing the entry below said: retired material is indistinguishable from
...
the entries below are readings of the ranking rather than of what a caller sees.
