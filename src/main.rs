...
fn cmd_query(
    ...
    loop {
        ...
        if merged.is_empty() {
            ...
            if !preds.is_empty() {
                // Opened again rather than held: the refresh branch above gives
                // the store up before re-indexing, and this path is reached
                // only when the answer was empty anyway.
                let st = Store::open(&root)?;
                for text in blames(&st.slots_with_fm()?, &preds) {
